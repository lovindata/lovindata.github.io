...
```sh
docker build --target backend_core -t lovindata/private:event_bus_backend_core-local -f 'Dockerfile' ..
```

To investigate the content of the docker image:

```sh
docker run -it --rm --entrypoint /bin/sh lovindata/private:event_bus_backend_core-local
```
